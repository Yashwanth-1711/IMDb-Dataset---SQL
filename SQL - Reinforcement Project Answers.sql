...
Select g.genre, count(*) as totalmovies 
    ...
    m.id = r.movie_id
    where m.Year = 2017 and m.country = "USA" 
    and (r.total_votes > 1000) and 
...
Select Count(*) as totalmovies
	from movie m join ratings r on 
    m.id = r.movie_id
    where date_published between "2018-04-01" 
    and "2019-04-01" and r.median_rating = 8;
...
select m.production_company, 
	sum(r.total_votes) as totalvotes 
	from movie m join ratings r on
    m.id = r.movie_id 
...
Select m.title, m.country, (n.name) as Director_name, 
	m.date_published from movie m 
    join director_mapping dm on 
...
Select m.title, m.duration, g.genre, 
